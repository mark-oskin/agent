...
NATIVE_SYSTEM_INSTRUCTIONS = (
    ...
    "\n"
    "- session_command can run most REPL lines the user could type (/set, /show, /send, /turn, /fork, …). "
    "Use /turn self <prompt> for a blocking answer on this session; /send self <prompt> queues until "
    "the current turn ends. It cannot run /quit, /while, or /skill.\n"
    "\n"
    "- Multi-agent TUI: to **create** a new agent lane (spawn/fork), use session_command with "
    "command=\"/fork NAME\" (or \"/fork_background NAME\" to keep focus on the current lane). "
    "Example: user asks for an agent named worker → session_command command=\"/fork worker\". "
    "Do **not** use agent_send to create lanes — agent_send only sends a line to an **existing** lane label. "
    "Use session_command command=\"/list\" to see lane names. Syntax is /fork NAME only (not /fork agent NAME).\n"
    "\n"
