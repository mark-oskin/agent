...
MCP_REPL_HELP = (
    ...
    "Per session: when MCP is connected, discovered mcp_* tools are enabled by default in each new session.\n"
    "Use /mcp session off to opt out (fork copies the parent's opt-out and tool set). "
    "/mcp session on re-enables. /set tools lists MCP tools with on/off per session.\n\n"
    "Commands:\n"
    ...
    "  /mcp session on | off  Re-enable or opt out of MCP tools for this session (default: on when connected)\n"
